...
async def execute_report(client: TelegramClient, target: str, reason_id: int, 
    ...
    async def do_report(entity, message_ids=None):
        ...
        if message_ids:
            ...
            await client(ReportRequest(peer=input_peer, id=message_ids, reason=reason_class(), message=""))
        else:
            # ReportPeerRequest(peer, reason, message)  
            await client(ReportPeerRequest(peer=input_peer, reason=reason_class(), message=""))
