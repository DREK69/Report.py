...
# ALL 20 REPORT REASONS - PRESERVED COMPLETELY
REASON_MAP = {
...
# FIXED KEYBOARD FUNCTION - Using proper Telethon Button objects
def get_main_menu_keyboard(user_role: UserRole):
    if user_role == UserRole.OWNER:
        return [
            [Button.text("📊 My Statistics"), Button.text("🔧 Manage Sessions")],
            [Button.text("📝 Start Reporting"), Button.text("🗂 All Sessions Report")],
            [Button.text("👥 User Management"), Button.text("⚙️ Bot Settings")],
            [Button.text("📈 View All Stats"), Button.text("❌ Cancel")]
        ]
    elif user_role == UserRole.APPROVED_USER:
        return [
            [Button.text("📊 My Statistics"), Button.text("🔧 Manage Sessions")],
            [Button.text("📝 Start Reporting"), Button.text("❌ Cancel")]
        ]
    else:
        return [
            [Button.text("ℹ️ Help"), Button.text("❌ Cancel")]
        ]
...
def get_session_selection_keyboard(sessions: List[str]):
    ...
    for i in range(0, len(sessions), 2):
        row = []
        row.append(Button.text(f"📱 {sessions[i][:20]}"))
        if i + 1 < len(sessions):
            row.append(Button.text(f"📱 {sessions[i+1][:20]}"))
        buttons.append(row)
    buttons.append([Button.text("✅ Select All Sessions")])
    buttons.append([Button.text("✔️ Done (Continue)"), Button.text("🔙 Back to Menu")])
    return buttons
...
def get_reason_keyboard():
    ...
    for num, (reason_text) in REASON_DISPLAY_MAP.items():
        row.append(Button.text(f"{num}. {reason_text[:15]}"))
        if len(row) == 2:
            buttons.append(row)
            row = []
    if row:
        buttons.append(row)
    buttons.append([Button.text("🔙 Back to Menu")])
    return buttons
...
async def message_handler(event):
    ...
    if user_id in user_states:
        ...
        if current_state == 'selecting_sessions':
            ...
            result_text = f"✅ **Report Operation Complete**\n\n"
            result_text += f"Target: `{target}`\n"
            result_text += f"Reason: {reason_name}\n"
            result_text += f"Success: {success_count}/{len(selected_sessions)}\n"
            result_text += f"Failed: {failed_count}/{len(selected_sessions)}\n"
            result_text += f"\nYour Total Reports: {users_db.get_user(user_id).total_reports}"
        ...
        if file_name.endswith('.zip'):
            ...
            with zipfile.ZipFile(zip_buffer, 'r') as zip_ref:
                ...
                for session_file in session_files:
                    ...
                    if session_file not in user.sessions:
                        user.sessions.append(session_file)
                        added_count += 1
                users_db.save_users()
            await event.respond(f"✅ Extracted {added_count} new session(s) from ZIP!\n\nTotal sessions: {len(user.sessions)}", buttons=get_main_menu_keyboard(role))
