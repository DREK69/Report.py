...
# Report delay between consecutive reports (in seconds)
REPORT_DELAY = 0.5  # Adjust as needed (0.1 to 5.0 seconds)
...
class ReportTask:
    ...
    retry_count: int = 0
    max_retries: int = 3
    use_all_sessions: bool = False  # New: Use all available sessions
...
class ReportQueue:
    ...
    def add_task(self, user_id: int, target: str, reason_id: int, count: int, 
                 session_id: Optional[str] = None, priority: int = 1, notes: str = "",
                 use_all_sessions: bool = False) -> str:
        ...
        self.save_queue()
        logger.info(f"Added task {task_id}: {target} x{count} (all_sessions={use_all_sessions})")
        return task_id
            ...
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                for file_info in zip_ref.filelist:
                    if file_info.filename.endswith('.session'):
                        ...
                        # ✅ FIX: Register session in session_manager
...
class URLParser:
    """Parse all Telegram URL formats"""
    ...
    def parse_url(url: str) -> dict:
        ...
        if match:
            ...
            result['message_id'] = int(match.group(2))
            result['type'] = 'message'
            return result
        ...
        if match:
            result['chat_id'] = int('-100' + match.group(1))
            result['message_id'] = int(match.group(2))
            result['type'] = 'message'
            return result
        
        # https://t.me/joinchat/HASH or https://t.me/+HASH
        match = re.match(r'https?://t\.me/(?:joinchat/|\+)([A-Za-z0-9_-]+)', url)
        if match:
        ...
        # Chat ID
        if url.startswith('-100') and url[1:].isdigit():
            result['chat_id'] = int(url)
            result['type'] = 'channel'
            return result
        
        return result

async def execute_report(client: TelegramClient, target: str, reason_id: int, 
                        disable_auto_join: bool = False) -> Tuple[bool, str]:
    """Advanced reporting with URL parsing, message support, auto join/leave"""
    from telethon.tl.functions.channels import JoinChannelRequest, LeaveChannelRequest
    from telethon.tl.functions.messages import ImportChatInviteRequest
    from telethon.tl.types import InputUser, InputChannel, InputPeerChat
    ...
    reason_desc  = REASON_MAP[reason_id][2]
    parsed = URLParser.parse_url(target)
    
    if not parsed['type']:
        return False, "Invalid target format"

    # --- Convert full entity (User/Channel/Chat) to InputPeer for RPCs ---
    async def to_input_peer(entity):
    ...
    # --- Resolve target string to full entity ---
    async def resolve_entity():
        if parsed['username']:
            return await client.get_entity(parsed['username'])
        elif parsed['chat_id']:
            return await client.get_entity(parsed['chat_id'])
        elif parsed.get('original'):
            return await client.get_entity(parsed['original'])
        return None

    # --- Fire the actual RPC (always converts to InputPeer first) ---
    async def do_report(entity, message_ids=None):
        input_peer = await to_input_peer(entity)
        if message_ids:
            # ReportRequest(peer, id, reason, message) - message as bytes
            await client(ReportRequest(input_peer, message_ids, reason_class(), b""))
        else:
            # ReportPeerRequest(peer, reason, message) - message as bytes
            await client(ReportPeerRequest(input_peer, reason_class(), b""))

    # ============================================================
    # disable_auto_join=True  — just report, no join/leave
    # ============================================================
    if disable_auto_join:
        try:
            ...
            if parsed['type'] == 'message':
                await do_report(entity, message_ids=[parsed['message_id']])
                return True, f"Reported message {parsed['message_id']}"
            else:
                await do_report(entity)
    ...
    # ============================================================
    # Full auto join/leave flow
    # ============================================================
    joined = False
    entity_to_leave = None
    ...
    try:
        ...
        if parsed['type'] == 'invite':
            try:
                ...
                joined = True
                entity_to_leave = entity
            except UserAlreadyParticipantError:
                entity = await client.get_entity(parsed['original'])
            except (InviteHashExpiredError, InviteHashInvalidError):
                return False, "Invalid/expired invite"

            await do_report(entity)

            if joined and entity_to_leave:
                await client(LeaveChannelRequest(entity_to_leave))
            return True, "Reported (joined+left)"

        # --- message report ---
        elif parsed['type'] == 'message':
            entity = await resolve_entity()
            if not entity:
                return False, "Invalid message target"

            if isinstance(entity, Channel):
                try:
                    await client.get_messages(entity, ids=parsed['message_id'])
                    ...
                    try:
                        ...
                        return False, "Cannot access private channel"
        ...
        logger.error(f"execute_report error: {type(e).__name__}: {e}")
        print(f"ERROR in execute_report: {type(e).__name__}: {e}")
        if joined and entity_to_leave:
            try:
                await client(LeaveChannelRequest(entity_to_leave))
            except:
...
async def process_report_task(task: ReportTask):
    ...
    # 🆕 MULTI-SESSION PARALLEL REPORTING
    if task.use_all_sessions:
        logger.info(f"Task {task.task_id}: Using ALL available sessions")
        
        all_sessions = session_manager.list_sessions()
        available_sessions = [s for s in all_sessions if s.status == "idle"]
        
        if not available_sessions:
            report_queue.update_task(
                task.task_id,
                status="failed",
                error_message="No available sessions",
                finished_at=datetime.now().isoformat()
            )
            stats_manager.record_task_completion(False)
            return
        
        # PRE-JOIN once for all sessions
        from telethon.tl.functions.channels import JoinChannelRequest, LeaveChannelRequest
        from telethon.tl.functions.messages import ImportChatInviteRequest
        from telethon.errors import UserAlreadyParticipantError, InviteHashExpiredError
        
        parsed = URLParser.parse_url(task.target)
        join_results = []
        
        # PRE-CHECK: For public channels, no join needed
        # Just add all sessions to join_results
        for session_config in available_sessions:
            client = await session_manager.get_client(session_config.session_id)
            if not client:
                logger.error(f"Failed to get client for {session_config.session_id}")
                continue
            
            # Add to results - will handle access in execute_report
            join_results.append((session_config, client, None, False))
            logger.info(f"Session ready: {session_config.session_id}")
        
        if not join_results:
            report_queue.update_task(
                task.task_id,
                status="failed",
                error_message="No sessions available",
                finished_at=datetime.now().isoformat()
            )
            stats_manager.record_task_completion(False)
            return
        
        # Now report with all sessions (no join/leave)
        async def report_with_session(session_config, client):
            session_manager.update_session_status(session_config.session_id, "active")
            session_success = 0
            session_failed = 0
            
            # Loop task.count times
            for i in range(task.count):
                success, message = await execute_report(
                    client, task.target, task.reason_id,
                    disable_auto_join=True  # Already joined!
                )
                
                if success:
                    session_success += 1
                    session_manager.increment_reports(session_config.session_id, True)
                    logger.info(f"Task {task.task_id}: Session {session_config.session_id} - Report {i+1}/{task.count} SUCCESS")
                else:
                    session_failed += 1
                    session_manager.increment_reports(session_config.session_id, False)
                    logger.error(f"Task {task.task_id}: Session {session_config.session_id} - Report {i+1}/{task.count} FAILED: {message}")
                
                # Small delay between reports from same session
                if i < task.count - 1:
                    await asyncio.sleep(REPORT_DELAY)
            
            session_manager.update_session_status(session_config.session_id, "idle")
            return {'success': session_success, 'failed': session_failed, 'session': session_config.session_id}
        
        # Run all reports in parallel
        report_tasks = [report_with_session(jr[0], jr[1]) for jr in join_results]
        results = await asyncio.gather(*report_tasks, return_exceptions=True)
        
        completed = sum(r.get('success', 0) for r in results if not isinstance(r, Exception))
        failed = sum(r.get('failed', 0) for r in results if not isinstance(r, Exception))
        ...
        report_queue.update_task(
            ...
            failed=failed,
            finished_at=datetime.now().isoformat(),
            notes=f"Multi-session: {completed}/{len(results)} successful"
        )
        
        duration = time.time() - start_time
        logger.info(f"Task {task.task_id} finished: {completed} completed, {failed} failed in {duration:.2f}s (multi-session)")
        stats_manager.record_task_completion(True)
        return
    
    # SINGLE SESSION REPORTING (original code)
    ...
    # 🆕 PRE-JOIN: Join once before reporting loop
    from telethon.tl.functions.channels import JoinChannelRequest, LeaveChannelRequest
    from telethon.tl.functions.messages import ImportChatInviteRequest
    from telethon.errors import UserAlreadyParticipantError, InviteHashExpiredError, InviteHashInvalidError
    ...
    try:
        ...
        if parsed['type'] == 'invite' and parsed['invite_hash']:
            try:
                result = await client(ImportChatInviteRequest(parsed['invite_hash']))
                joined_entity = result.chats[0]
                needs_leave = True
            except UserAlreadyParticipantError:
                joined_entity = await client.get_entity(parsed['original'])
            except (InviteHashExpiredError, InviteHashInvalidError):
                report_queue.update_task(
                    task.task_id,
                    status="failed",
        ...
        # Try to join if private channel
        elif parsed['type'] in ['message', 'channel']:
            entity = None
            if parsed['username']:
        ...
        logger.warning(f"Pre-join check failed: {e}")
    
    completed = 0
    failed = 0
    
    # Report loop - NO join/leave inside
    for i in range(task.count):
        # Pass disable_auto_join=True to prevent join/leave in execute_report
        success, message = await execute_report(
            client, task.target, task.reason_id,
            disable_auto_join=True  # New parameter
        )
    ...
    # 🆕 POST-LEAVE: Leave once after all reports
    if needs_leave and joined_entity:
        try:
            await client(LeaveChannelRequest(joined_entity))
            logger.info(f"Left channel after completing {completed} reports")
        except Exception as e:
            logger.warning(f"Failed to leave: {e}")
...
def get_admin_menu_buttons():
    return [
        ...
        [Button.inline("⚙️ Settings", b"admin_settings"), Button.inline("📤 Export", b"admin_export")],
        [Button.inline("📥 Import Sessions", b"admin_import")],
    ...
    if event.is_private and not event.message.message.startswith('/'):
        ...
        if event.message.document:
            if not user_state_manager.is_admin(user_id):
                await event.respond("❌ Only admins can upload session files")
                return
            ...
            try:
                ...
                if result['success']:
                    ...
                    await event.respond(
                        response,
                        buttons=[[Button.inline("📱 View Sessions", b"admin_sessions")],
        ...
        if state.current_action == "awaiting_target":
            ...
            try:
                ...
                buttons = [
                    [Button.inline("🔄 Auto Select", b"session_auto")],
                    [Button.inline("🚀 Use ALL Sessions", b"session_all")]  # NEW!
                ]
                ...
                text = "**Select Session:**\n\n"
                text += "🔄 Auto: Single session\n"
                text += "🚀 ALL: Parallel with all sessions"
...
async def session_select_handler(event):
    ...
    # Handle multi-session option
    use_all_sessions = False
    if session_data == "all":
        session_id = None
        use_all_sessions = True
    elif session_data == "auto":
        session_id = None
    else:
    ...
    task_id = report_queue.add_task(
        user_id, target, reason_id, count, session_id,
        use_all_sessions=use_all_sessions
    )
...
async def admin_import_handler(event):
    ...
    help_text = """📥 **Import Sessions**

**Supported formats:**
• **.session** - Single Telegram session file
• **.zip** - Multiple .session files
...
async def scan_uploads_handler(event):
    ...
    text = f"📥 **Scan Results** ({len(results)}):\n\n"
    
    for item in results:
        text += f"📄 **{item['file']}**\n"
        if item['result']['success']:
            text += f"✅ {item['result']['message']}\n"
        else:
            text += f"❌ {item['result']['message']}\n"
        text += "\n"
    ...
    await event.edit(
        text[:4000],
        buttons=[[Button.inline("🔙 Back", b"admin_import")]]
    )


@bot.on(events.CallbackQuery(pattern=b"admin_settings"))
...
📊 **Report Delay:** {REPORT_DELAY} seconds
└ Delay between consecutive reports

💡 To change delay, use:
`/set_delay <seconds>`

Example: `/set_delay 1.0`
Range: 0.1 - 5.0 seconds
"""
    
    await event.edit(text, buttons=[[Button.inline("🔙 Admin Panel", b"admin_panel")]])

@bot.on(events.NewMessage(pattern=r'/set_delay (\d+\.?\d*)'))
async def set_delay_handler(event):
    ...
    try:
        global REPORT_DELAY
        new_delay = float(event.pattern_match.group(1))
        
        if new_delay < 0.1 or new_delay > 5.0:
            await event.reply("❌ Delay must be between 0.1 and 5.0 seconds")
            return
        
        REPORT_DELAY = new_delay
        await event.reply(f"✅ Report delay set to {REPORT_DELAY} seconds")
    except:
        await event.reply("❌ Invalid format. Use: /set_delay 1.0")
