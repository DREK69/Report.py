...
async def execute_report(client: TelegramClient, target: str, reason_id: int, 
    ...
    async def do_report(entity, message_ids=None):
        ...
        if message_ids:
            # Telethon 1.42: ReportRequest(peer, id, reason)
            await client(ReportRequest(input_peer, message_ids, reason_class()))
        else:
            # Telethon 1.42: ReportPeerRequest(peer, reason)  
            await client(ReportPeerRequest(input_peer, reason_class()))
