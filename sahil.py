    ...
    if task.use_all_sessions:
        ...
        async def report_with_session(session_config, client):
            ...
            if success:
                ...
                logger.info(f"Task {task.task_id}: Session {session_config.session_id} - SUCCESS")
            else:
                session_manager.increment_reports(session_config.session_id, False)
                logger.error(f"Task {task.task_id}: Session {session_config.session_id} - FAILED: {message}")
            
            return {'success': success, 'session': session_config.session_id, 'message': message}
...
def get_admin_menu_buttons():
    return [
        ...
        [Button.inline("📥 Import Sessions", b"admin_import"), Button.inline("📤 Export", b"admin_export")],
