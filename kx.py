...
async def get_entity_type(entity):
...
async def get_entity_from_target(client,target):
    ...
    return entity
async def report_target(session_path,target,reason_obj,phone,uid):
    client=None
    try:
        is_waiting,wait_time=check_flood_wait(phone)
        if is_waiting:
            return False,"FloodWait",f"FloodWait:{wait_time}s"
        ...
        if not entity:
            await client.disconnect()
            return False,"unknown","Could not get entity"
        entity_type=await get_entity_type(entity)
        ...
        if client:
            try:
                ...
                pass
        return False,"unknown",f"Flood:{e.seconds}s"
    except Exception as e:
        if client:
            try:
                await client.disconnect()
                ...
                pass
        error_str=str(e)[:50]
        return False,"unknown",error_str
async def report_messages(session_path,target,msg_ids,reason_obj,phone,uid):
    client=None
    try:
        is_waiting,wait_time=check_flood_wait(phone)
        if is_waiting:
            return False,"unknown",f"FloodWait:{wait_time}s"
        client=TelegramClient(session_path,API_ID,API_HASH)
        await client.connect()
        if not await client.is_user_authorized():
            await client.disconnect()
...
async def report_menu(event):
    ...
    buttons=[]
    if sessions:
        buttons.append([Button.inline("👤 User",b"report_user")])
        buttons.append([Button.inline("📢 Channel",b"report_channel")])
        buttons.append([Button.inline("👥 Group",b"report_group")])
...
async def export_callback(event):
    ...
    try:
        ...
        await bot.send_file(uid,zip_name,caption=f"✅ {len(sessions)} sessions")
        await msg.delete()
    except Exception as e:
...
async def report_channel_callback(event):
    uid=event.sender_id
...
async def report_group_callback(event):
    uid=event.sender_id
...
async def set_delay_callback(event):
    ...
    await event.respond("⏱ Delay (1-30s):",buttons=[[Button.inline("« Cancel",b"menu_settings")]])
@bot.on(events.CallbackQuery(pattern=b"set_limit"))
async def set_limit_callback(event):
    uid=event.sender_id
    set_state(uid,'awaiting_limit')
    await event.respond("📊 Limit (1-100):",buttons=[[Button.inline("« Cancel",b"menu_settings")]])
@bot.on(events.CallbackQuery(pattern=b"set_reports_per_target"))
async def set_reports_per_target_callback(event):
    uid=event.sender_id
    set_state(uid,'awaiting_reports_per_target')
    await event.respond("🔢 Reports per target (1-50):",buttons=[[Button.inline("« Cancel",b"menu_settings")]])
...
async def reason_callback(event):
    ...
    if state['state']=='awaiting_reason':
        ...
        await event.respond(f"✅ {reason_name}\n\nSession count (0=all):",buttons=[[Button.inline("« Cancel",b"menu_report")]])
...
async def add_via_phone(event):
    ...
    await event.respond("📱 Phone:\n\n+1234567890",buttons=[[Button.inline("« Cancel",b"menu_sessions")]])
...
async def text_handler(event):
    ...
    if state['state']=='awaiting_phone':
        ...
        try:
            ...
            await client.disconnect()
            await event.respond(f"❌ {str(e)[:100]}")
            clear_state(uid)
        ...
        try:
            ...
            await msg.edit(f"❌ {str(e)[:100]}")
            clear_state(uid)
    elif state['state'] in ['awaiting_user_target','awaiting_channel_target','awaiting_group_target']:
        targets=[l.strip() for l in text.split('\n')if l.strip()]
        if not targets:
            await event.respond("❌ No targets")
            clear_state(uid)
            return
        ...
        try:
            ...
            await process_target_report(event,state['targets'],state)
        except ValueError:
        ...
        try:
            ...
            if 1<=delay<=30:
                ...
                clear_state(uid)
                await event.respond(f"✅ Delay: {delay}s",buttons=[[Button.inline("⚙ Settings",b"menu_settings")]])
            else:
                await event.respond("❌ 1-30")
        except ValueError:
            await event.respond("❌ Invalid")
        ...
        try:
            ...
            if 1<=limit<=100:
                ...
                clear_state(uid)
                await event.respond(f"✅ Limit: {limit}",buttons=[[Button.inline("⚙ Settings",b"menu_settings")]])
            else:
                await event.respond("❌ 1-100")
        except ValueError:
            await event.respond("❌ Invalid")
    elif state['state']=='awaiting_reports_per_target':
        try:
            rpt=int(text)
            if 1<=rpt<=50:
                update_setting(uid,'reports_per_target',rpt)
                clear_state(uid)
                await event.respond(f"✅ Reports/Target: {rpt}",buttons=[[Button.inline("⚙ Settings",b"menu_settings")]])
            else:
                await event.respond("❌ 1-50")
        except ValueError:
            await event.respond("❌ Invalid")
async def process_target_report(event,targets,state):
    uid=event.sender_id
    ...
    success,failed=0,0
    total_ops=len(targets)*len(selected)*reports_per_target
    processed=0
    for target in targets:
        for rep_num in range(reports_per_target):
            for s in selected:
                path=os.path.join('sessions_db',s['session_file'])
                ok,entity_type,error=await report_target(path,target,reason_obj,s['phone'],uid)
                if ok:
                    success+=1
                    update_session_stats(uid,s['phone'],success=1)
                    log_report(uid,s['phone'],target,entity_type,reason_name,1)
                else:
                    failed+=1
                    update_session_stats(uid,s['phone'],failed=1)
                    log_report(uid,s['phone'],target,entity_type,reason_name,0,error or '')
                processed+=1
                if processed%5==0 or processed==total_ops:
                    progress=f"🎯 {processed}/{total_ops}\n✅ {success} ❌ {failed}"
                    try:
...
async def process_message_report(event,target,msg_ids,state):
    uid=event.sender_id
    msg=await event.respond(f"🎯 Reporting {len(msg_ids)} msgs...")
    sessions=get_sessions(uid)
    verified=[s for s in sessions if s['verified']]
    if not verified:
    ...
    reason_name=state['reason_name']
    success,failed=0,0
    total_ops=len(selected)*reports_per_target
    processed=0
    for rep_num in range(reports_per_target):
        for s in selected:
            path=os.path.join('sessions_db',s['session_file'])
            ok,entity_type,error=await report_messages(path,target,msg_ids,reason_obj,s['phone'],uid)
            if ok:
                success+=1
                update_session_stats(uid,s['phone'],success=1)
                log_report(uid,s['phone'],target,entity_type,reason_name,1)
...
def main():
    print("╔════════════════════════════════════════╗")
    print("║   🎯 ADVANCED TELEGRAM REPORTER 🎯  ║")
    print("║      Complete System with Groups      ║")
    print("╚════════════════════════════════════════╝")
    print()
    print("✅ Running...")
    ...
    print()
    print("🔥 Features:")
    print("   • User/Channel/Group reporting")
    print("   • Reports per target control")
    print("   • Multi-session support")
    print("   • Private/Public group support")
    print("   • Auto-join for private links")
    print("   • Message range reporting")
    print("   • Entity type detection")
