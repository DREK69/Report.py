...
# Reason classes that require a comment argument in their constructor
_REASON_NEEDS_COMMENT = {
    InputReportReasonOther,
    InputReportReasonPersonalDetails,
}

def _build_reason(reason_id: int) -> "InputReportReason*":
    """Instantiate the correct reason object.
    Other & PersonalDetails require comment=; the rest take none."""
    _cls = REASON_MAP[reason_id][1]
    _desc = REASON_MAP[reason_id][2]          # human description from the map
    if _cls in _REASON_NEEDS_COMMENT:
        return _cls(comment=_desc)
    return _cls()
...
async def execute_report(client: TelegramClient, target: str, reason_id: int, 
    ...
    reason_instance = _build_reason(reason_id)
    parsed = URLParser.parse_url(target)
    
    if not parsed['type']:
        return False, "Invalid target format"
    
    # Skip join/leave if disabled (already joined in process_report_task)
    if disable_auto_join:
        try:
            # Just report, no join/leave
            if parsed['type'] == 'message':
                ...
                if parsed['username']:
                    ...
                    entity = await client.get_entity(parsed['chat_id'])
                elif parsed['invite_hash']:
                    # Already joined, get entity from members
                    entity = await client.get_entity(parsed['original'])
                else:
                    return False, "Invalid target"
                
                await client(ReportPeerRequest(entity, reason_instance, ""))
    ...
    # Original auto join/leave code (when disable_auto_join=False)
    joined = False
...
bot = TelegramClient('reporter_bot', BOT_API_ID, BOT_API_HASH)
