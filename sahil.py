...
async def process_report_task(task: ReportTask):
    ...
    if task.use_all_sessions:
        ...
        for session_config in available_sessions:
            ...
            try:
                if parsed['type'] == 'invite' and parsed['invite_hash']:
                    try:
                        result = await client(ImportChatInviteRequest(parsed['invite_hash']))
                        join_results.append((session_config, client, result.chats[0], True))
                    except UserAlreadyParticipantError:
                        entity = await client.get_entity(parsed['original'])
                        join_results.append((session_config, client, entity, False))
                    ...
                    if entity and isinstance(entity, Channel):
                        try:
                            ...
                            try:
                                await client(JoinChannelRequest(entity))
                                join_results.append((session_config, client, entity, True))
                            except:
                                pass
                    else:
                        join_results.append((session_config, client, None, False))
            except Exception as e:
                logger.warning(f"Join failed for {session_config.session_id}: {e}")
        ...
        # POST-LEAVE: Leave all channels
        for session_config, client, entity, needs_leave in join_results:
            if needs_leave and entity:
                try:
                    await client(LeaveChannelRequest(entity))
                except Exception as e:
                    logger.warning(f"Leave failed for {session_config.session_id}: {e}")
