...
async def process_report_task(task: ReportTask):
    ...
    if task.use_all_sessions:
        ...
        async def report_with_session(session_config):
            client = await session_manager.get_client(session_config.session_id)
            if not client:
                return {'success': False, 'session': session_config.session_id, 'message': 'Connection failed'}
        ...
        # Run all in parallel
        results = await asyncio.gather(*[report_with_session(s) for s in available_sessions], return_exceptions=True)
