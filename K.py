...
class DB:
    ...
    def migrate_db(self):
        ...
        try:
            ...
            self.conn.commit()
        try:
            c.execute("SELECT session_count FROM settings LIMIT 1")
        except sqlite3.OperationalError:
            c.execute("ALTER TABLE settings ADD COLUMN session_count INTEGER DEFAULT 0")
            self.conn.commit()
        try:
            c.execute("SELECT random_order FROM settings LIMIT 1")
        except sqlite3.OperationalError:
            c.execute("ALTER TABLE settings ADD COLUMN random_order INTEGER DEFAULT 1")
            self.conn.commit()
        try:
            c.execute("SELECT retry_failed FROM settings LIMIT 1")
        except sqlite3.OperationalError:
            c.execute("ALTER TABLE settings ADD COLUMN retry_failed INTEGER DEFAULT 0")
            self.conn.commit()
        try:
            c.execute("SELECT error_msg FROM reports LIMIT 1")
        except sqlite3.OperationalError:
            c.execute("ALTER TABLE reports ADD COLUMN error_msg TEXT")
            self.conn.commit()
